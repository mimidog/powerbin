...
// Do XML HTTP request
	_doRequest: function(anEvent, aQuery) {
	    ...
	    if(PL.lbm_req.length > 0) {  
	            url += "&lbm_req"  + "=" + PL.lbm_req;  
	    }  
	    if(PL.threadNum>0){
	    	 url += "&threadNum"  + "=" + PL.threadNum; 
	    }
		PL.debug('_doRequest', url);
		PL._getResponseXML(url, PL._onResponse);
...
// On response  optional callback fun is called with optional user data.
	_getXML: function(url, callback) {
		...
		// Open URL
//		xmlhttp.open('post', url.split("?")[0], async);
...
//		xmlhttp.send(url.split("?")[1]);
	...
	//用ajax代替原生写法
	_getResponseXML:function(url, callback){
		var cb = callback;
		var async = false;
		$.ajax({
			url:url.split("?")[0],
			data:getJsonFromStr(url.split("?")[1]),
//			dataType:"json",
			type:"post",
...
/** Send debug messages to a (D)HTML window. */
	debug: function(label, value) {
		...
		PL.debugWindow.scrollTo(PL.debugWindow.scrollX,100000);
...
function onData(event){
	var msg=event.get("val");
 if(top && top.kui_debugger)
	top.kui_debugger.debug("pushlet返回参数",JSON.stringify(msg));
	PL.callBack(msg);
	//callBack.call(PL,msg);
}
...
function requestLbm(){
	$.ajax({
		...
		data:{
			lbm_req:PL.lbm_req,
			threadNum:PL.threadNum,
			clientId:PL.sessionId
		},
		type:"post"
		});
}
...
function getJsonFromStr(str){
	...
	for(var i=0;i<array.length;i++){
		var temp=array[i];
		json[temp.split("=")[0]]=temp.split("=")[1];
	}
	return json;
}
/**
...
var plRequest={
	...
	request:function(con){
		isPushletLeave = false;
		var req={
			lbm_req:con.lbm_req
		}
		if(top && top.kui_debugger)
			top.kui_debugger.debug("pushlet请求参数",JSON.stringify(con));
		PL.lbm_req=encrypt(JSON.stringify(req));
		PL.threadNum=con.threadNum;
		PL._init();
		PL.joinListen("/plPush");
		//PL.setDebug(true);
		PL.callBack=con.callBack;
	}
}
//indicate that the flag of pushlet leave event is runned or not 
var isPushletLeave=false;