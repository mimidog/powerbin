...
var SimpleCalendar = function () {
  ...
  function SimpleCalendar(query, options) {
    ...
    this.create(this.tyear,this.tmonth);
  }
  ...
  _createClass(SimpleCalendar, [{
    ...
    key: 'create',
    value: function create(tyear,tmonth) {
    ...
    value: function update() {
      ...
      var  year = arguments.length <= 1 || arguments[1] === undefined ? this.defaultYear : arguments[1];
      this.updateSize();
      this.updateWeek();
      ...
      this.updateSelect(year, month);
    }
    //根据当前日期,刷新日历头年份,月份默认选中
  },{
      key: 'updateTitle',
      value: function updateTitle() {
          var month = arguments.length <= 0 || arguments[0] === undefined ? this.tmonth : arguments[0];
          var year = arguments.length <= 1 || arguments[1] === undefined ? this.tyear : arguments[1];
          $('.sc-select-month').val(parseInt(this.tmonth));
          $('.sc-select-year').val(parseInt(this.defaultYear));
    ...
    value: function updateSelect(year, month) {
      ...
      for (var i = this.timeRange.startYear; i < this.timeRange.endYear + 1; i++) {
    ...
    value: function updateHoliday(year, month) {
      ...
      if (options.showHoliday) {
        ...
        var data = this.languageData.vocation['newest_year_data'];
        if (data) {
          daysElement.forEach(function (v, i) {
            var day = +v.querySelector('.day').innerHTML;
            if (day == 1) currentmonth++;
    ...
    value: function updateEvent() {
      ...
      yearadd.onclick = function () {
        ...
        if (currentyear < calendar.timeRange.endYear) currentyear++;
        selectYear.value = currentyear;
        //年份更新后，刷新当前年份节假日信息
        refreshYearHolidays(currentyear);
        calendar.update(this.tmonth, currentyear);
      };
      yearsub.onclick = function () {
        var currentyear = selectYear.value;
        if (currentyear > calendar.timeRange.startYear) currentyear--;
        selectYear.value = currentyear;
        //年份更新后，刷新当前年份节假日信息
        refreshYearHolidays(currentyear);
        calendar.update(this.tmonth, currentyear);
      ...
      monthadd.onclick = function() {
        ...
        if (currentmonth < 12) currentmonth++;else {
          if(currentyear == calendar.timeRange.endYear){
            return ;
          }
          currentmonth = 1;
          selectYear.value = ++currentyear;
          //年份更新后，刷新当前年份节假日信息
          refreshYearHolidays(selectYear.value);
        };
      ...
      monthsub.onclick = function () {
        ...
        if (currentmonth > 1) currentmonth--;else {
            if(currentyear == calendar.timeRange.startYear){
                return ;
            }
          currentmonth = 12;
          selectYear.value = --currentyear;
      ...
      returntoday.onclick = function () {
        ...
        //当前选择年份与返回年份不相等时
        if(selectYear.value != calendar.tyear){
...
SimpleCalendar.prototype.timeRange = {
        startYear : 2049,
        endYear : 2049
}
...
/*
  ...
  功能：刷新选择年份节假日信息
 */
function refreshYearHolidays(year) {
    //查询当前选择年份中的节假日信息
    ...
    //刷新当前年份节假日信息
    myCalendar.languageData.vocation.newest_year_data = formatHolidays(selectYearHolidays);
}
