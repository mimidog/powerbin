...
//    console.log(arguments.callee.$.construct.apply(this, arguments));
		arguments.callee.$.construct.apply(this, arguments);
//    console.log("20-38-end");
	...
	initializeRules: function(stencilSet) {
		...
		if (existingSS) {
			...
			var extensionValues = [],extensions = stencilSet.extensions();
      for (var property in extensions) extensionValues.push(extensions[property]);
			jQuery.each(extensionValues,function(i,extension) {
				if(extension.rules) {
      ...
      var $this = this;
			if (jsonRules.connectionRules) {
				jQuery.each(jsonRules.connectionRules,(function(i, rules){
					...
					jQuery.each(rules.connects,(function(j,connect){
						var toRoles = [];
						if (connect.to) {
							if (!(connect.to instanceof Array)) {
								connect.to = [connect.to];
							}
							jQuery.each(connect.to,(function(k,to){
			...
			if (jsonRules.cardinalityRules) {
				jQuery.each(jsonRules.cardinalityRules,(function(i,rules){
					var cardrKey;
					if ($this._isRoleOfOtherNamespace(rules.role)) { cardrKey = rules.role; }else{ cardrKey = namespace + rules.role;
          }
          if (!cardr[cardrKey]) { cardr[cardrKey] = {}; for(i in rules) { cardr[cardrKey][i] = rules[i]; }
          }
          var oe = {};
					if (rules.outgoingEdges) {
						jQuery.each(rules.outgoingEdges,(function(j,rule){
					...
					if (rules.incomingEdges) {
						jQuery.each(rules.incomingEdges,(function(k,rule){
							if ($this._isRoleOfOtherNamespace(rule.role)) {
								ie[rule.role] = rule;
							}
							else {
			...
			if (jsonRules.containmentRules) {
				jQuery.each(jsonRules.containmentRules,(function(i,rules){
					var conrKey;
					if ($this._isRoleOfOtherNamespace(rules.role)) { conrKey = rules.role; }else{ $this._containerStencils.push(namespace + rules.role); conrKey = namespace + rules.role; } if(!conr[conrKey]) { conr[conrKey] = []; }
          jQuery.each((rules.contains||[]),(function(j,containRole){
			...
			if (jsonRules.morphingRules) {
				jQuery.each(jsonRules.morphingRules,(function(i,rules){
					var morphrKey;
					if ($this._isRoleOfOtherNamespace(rules.role)) { morphrKey = rules.role; }else{ morphrKey = namespace + rules.role; } if(!morphr[morphrKey]) { morphr[morphrKey] = []; } if(!rules.preserveBounds) { rules.preserveBounds = false; }
          jQuery.each(rules.baseMorphs,(function(j,baseMorphStencilId){
			...
			if (jsonRules.layoutRules) {
				...
				jQuery.each(jsonRules.layoutRules,function(i,rules){
					var layoutKey;
					if ($this._isRoleOfOtherNamespace(rules.role)) {
...
//    var a = false;
//    for(var i=0;i<this._stencils.length;i++){
//      if(this._stencils[i] && id == this._stencils[i].id()){
//        a = true;
//        break;
//      }
//    }
