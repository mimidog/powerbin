...
def closeJob():
    ...
    for stock in SEC_LIST:
        ...
        if C2 > 2 and (max(C0_DICT[stock][-6:-1]) < 2 or max(C0_DICT[stock][-7:-2]) < 2) and position_long == 0:
            # 补开仓前把之前的未成交的开仓单撤掉
            ...
            '''
        if C1 < -2 and (min(C0_DICT[stock][-6:-1]) > -2 or min(C0_DICT[stock][-7:-2]) > -2) and position_short == 0:
...
def checkTakeProfit():
    ...
    for stock in SEC_LIST:
        ...
        cost_long = positions[stock].position_cost_long * pos_long
        cost_short = positions[stock].position_price_short * pos_short
        ...
        if profit_long == profit_long:
            if profit_long > 0:
                if abs(profit_long) / cost_long > para['TP_RATIO1'] and pos_long == para['OPEN_NUM']:
                    ...
                    un_sell_close[stock].append(close_order1.order_id)
                elif abs(profit_long) / cost_long > para['TP_RATIO2'] and pos_long == (
            ...
            if profit_short > 0:
                if abs(profit_short) / cost_short > para['TP_RATIO1'] and pos_short == para['OPEN_NUM']:
                    ...
                    close_order1 = api.insert_order(symbol=stock, direction='BUY', offset=offsetStr, \
                                                    volume=para['CLOSE_NUM'], limit_price=(new_price + price_tick))
                    un_buy_close[stock].append(close_order1.order_id)
                elif abs(profit_short) / cost_short > para['TP_RATIO2'] and pos_short == (
                        para['OPEN_NUM'] - para['CLOSE_NUM']):
                    log.logger.info('{0}空单达到止盈线2位置!!!止盈减仓{1}手>>>'.format(stock, para['CLOSE_NUM2']))
                    close_order2 = api.insert_order(symbol=stock, direction='BUY', offset=offsetStr, \
                                                    volume=para['CLOSE_NUM2'], limit_price=(new_price + price_tick))
                    un_buy_close[stock].append(close_order2.order_id)
            elif profit_short < 0 and abs(profit_long) / cost_long > para['SP_RATIO']:
    ...
    while True:
        ...
        for sec in SEC_LIST:
            if api.is_changing(klines_dict[sec].iloc[-1], "datetime"):
                ...
                log.logger.info('{0}多头持仓成本: {1}, 多头持仓盈亏: {2}'.format(sec, positions[sec].position_cost_long, positions[sec].position_profit_long))
                log.logger.info('{0}空头持仓成本: {1}, 空头持仓盈亏: {2}'.format(sec, positions[sec].position_cost_short, positions[sec].position_profit_short))
                if para['SIGNAL_ID'] == 0:
                    run_trade(sec)
