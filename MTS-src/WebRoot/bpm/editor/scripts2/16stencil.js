...
ORYX.Core.StencilSet.Stencil = {
    ...
    var $this = this;
    jQuery.each(this._jsonStencil.roles,(function(index,role){
...
//		}).bind(this));
		...
		*/
		...
		if(this._jsonStencil.view.trim().match(/</)) {
			...
			if( ORYX.Editor.checkClassType( xml.documentElement, SVGSVGElement )) {
				...
				jQuery.each($A(imageElems),(function(i,imageElem) {
      ...
      jQuery.ajax({
        url:url,
        async:false,
        success:this._loadSVGOnSuccess.bind(this),
        error:this._loadSVGOnFailure.bind(this)
      });
//			new Ajax.Request(
//				url, {
