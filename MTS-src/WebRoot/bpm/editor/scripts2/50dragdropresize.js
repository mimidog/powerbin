...
ORYX.Plugins.GridLine = Clazz.extend({
	...
	getScale: function(){
		try {
			return this.parent[0].parentNode.transform.baseVal.getItem(0).matrix.a;
		} catch(e) {
			return 1;
		}
	},
	...
	update: function(pos) {
		...
		if (this.direction === ORYX.Plugins.GridLine.DIR_HORIZONTAL) {
			...
			var cWidth = this.parent[0].parentNode.parentNode.width.baseVal.value/this.getScale();
			this.line.setAttributeNS(null, 'd', 'M 0 '+y+ ' L '+cWidth+' '+y);
		} else {
			var x = pos instanceof Object ? pos.x : pos; 
			var cHeight = this.parent[0].parentNode.parentNode.height.baseVal.value/this.getScale();
