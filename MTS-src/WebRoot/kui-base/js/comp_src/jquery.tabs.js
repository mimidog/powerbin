...
(function($){
	...
	$.fn.tabs = function(options, param){
		if (typeof options == 'string') {
			return $.fn.tabs.methods[options].apply(this, [this].concat(Array.prototype.slice.call(arguments, 1)));
		}
	...
	$.fn.tabs.methods = {
		...
		select: function(jq, which, isId){
			return jq.each(function(){
				selectTab(this, which, isId);
			});
		},
		...
		addPageTab:function(jq, options){
			return jq.each(function(){
				addPageTab(this, options);
			});
		},

		getTabById: function(jq, id) {
			return getTabById(jq[0], id);
		}
	};
	...
	function wrapTabs(container) {
		...
		$.data(container, 'tabs').options.onInitSuccess.call(this);
	}
	...
	function createTab(container, pp, options) {
		...
		var idString = options.id && options.id != "" ?  " id='"+ options.id +"' " : "";
		opts.tab = $('<li '+idString+'></li>').appendTo(tabs);	// set the tab object in panel options
		opts.tab.append(
	...
	/**
	 * add frame tab page
	 * @param options {id:page_id,title:page_title,url:page_url}
	 */
	function addPageTab(container, options){
		if(!tabIdExists(container,options.id)) {
			addTab(container,{
				id:options.id,
				title:options.title,
				content:"<iframe id=\""+options.id+"\" nodeid=\""+options.id+"\" scrolling=\"auto\" frameborder=\"0\" allowtransparency=\"true\"  style=\"width:100%;height:99%;overflow:auto;\"></iframe>",
				closable:String(options.closable) != "undefined" ? options.closable : true
			});
			$("iframe#"+options.id,container).attr("src",options.url);
		}else{
			selectTab(container,options.id,true);
		}
	}
	
	function tabIdExists(container,id){
		return getTabById(container,id) != null;
	}
	
	function getTabById(container,id){
		var tabs = $.data(container, 'tabs').tabs;
	...
	function updateTab(container, param){
		...
		if (opts.closable){
			...
			var a_close = $('<a href="javascript:void(0)" class="tabs-close" title="关闭"></a>').appendTo(tab);
		...
		if(s_title.width()>=230){
			...
			s_title.attr("title",s_title.html());
		}
...
//		setProperties(container);
	...
	function selectTab(container, which, isId){
		var opts = $.data(container, 'tabs').options;
		var tabs = $.data(container, 'tabs').tabs;
		var selectHis = $.data(container, 'tabs').selectHis;
		...
		var panel;
		if (isId)
			panel = getTabById(container, which);
		else
			panel = getTab(container, which); // get the panel to be activated
		if (!panel) return;
