...
(function($) {
        ...
        if (methods[options]) {
			...
			return this.each(function(){
				methods.init.call(this, $.extend({},
					$.fn.uploadify.defaults, $.fn.uploadify.parseOptions(this), options));
			});
        } else {
    ...
    $.fn.uploadify.parseOptions = function(target) {
        var jq = $(target);
        return $.extend({},
        $.parser.parseOptions(target, ["id", "butonText", "fileTypeDesc", "fileTypeExts", "fileSizeLimit", {
            multi: "boolean"
        }]), {
            id: jq.attr("id")
        });
    ...
    $.fn.uploadify.defaults = {
        ...
        height: 30,	//浏览文件按钮的高度
        dir: '',	//读取文件
        readfile:'false',	 //附件保存文件夹名称 例如report,则附件上传后,放在/upload/report下,为空,则放在upload目录下
        //uploadUrl:'../../kjdp_upload?DIR=' + this.dir + '&READFILE='+this.readfile,		  //上传的路径
        buttonClass: '',	 //浏览文件按钮的样式
        buttonImage: '',	//如果浏览文件未选择样式,则使用该图片
        buttonText: '请选择文件',	// 选择文件按钮的文本描述
        // 浏览文件窗口的可选择文件类型的描述
        fileTypeDesc: '请选择doc,docx,xls,xlsx,pdf,txt,jpg,gif,jpge,ppt,rar,zip文件',
        // 浏览文件窗口允许选择文件的扩展名
        fileTypeExts: '*.doc;*.docx;*.xls;*.xlsx;*.pdf;*.txt;*.jpg;*.gif;*.jpge;*.ppt;*.rar;*.zip',
        //单个上传文件最大大小,可接受单位(B KB MB GB,如果设为0,则不限制)
        fileSizeLimit: '10MB',
        multi: false,		// 浏览文件窗口是否允许选择多个文件(多文件上传)
        IS_IMAGE_RESOURCE:'false',//是否上传到影像目录
        LOCAL_RESOURCE:'false',//是否直接上传相对路径
        UN_RENAME:'false',//上传文件无需重命名
        fileData: [],		// 
		fileNumLimit: 0,	//一次选择上传文件的数量
        onlyView: false,	//仅供下载  无上传按钮
        autoCheck: true,	//是否在初始化uploadify时检测flash版本
        showButton: true,	//是否在初始化uploadify时显示上传按钮   ok
        showQueue: true,	 //是否在初始化uploadify时显示文件上传界面  ok
        onDialogOpen: function() {},	//ok
        onInit: function() {},		   //ok
		onUploadSuccess:function(){},  //ok
		onDeleteSuccess:function(){}   //ok
    };
    ...
    var methods = {
        ...
        //flash 展示文件
        showFiles:function(fileData){
            var uploader = $.data($(this)[0], "uploader");
            $.isFunction(uploader.showFiles) && uploader.showFiles.call($(this),fileData);
        },
        //flash 仅供下载用,无上传按钮
        onlyView: function(flag){
            var uploader = $.data($(this)[0], "uploader");
            $.isFunction(uploader.onlyView) && uploader.onlyView($(this),flag);
        }
    ...
    Html5Upload.prototype.init = function(target,c){
    	...
    	d.createInput = function() {
    		...
    		fileInput.bind(
    			...
    			function() {
    				...
    				d.queue.selected = l;
    				//上传文件的数量判断
    				if ((d.queue.count + l) > f.fileNumLimit
    						&& f.fileNumLimit !== 0) {
    					if ($.inArray("onError",
    							f.overrideEvents) < 0) {
    						alert("选定的文件数量超过设置的上传文件数 ("
    								+ f.fileNumLimit
    								+ ")！");
    					}
    					...
    					var checkfileType = function(_file){
    						var result=true;
    						if(f.fileTypeExts){
    							var _fileType = _file.name.split(".")[1],
    					...
    					for (var m = 0; m < l; m++) {
    						...
    						if(!checkfileType(file)){
    							alert('这个文件 "' + file.name + '" 文件类型不支持 (' + f.fileTypeDesc + ').');
    							return false;
    						}
    						d.addQueueItem(file);
    					}
    	...
    	d.drop = function(m) {
    		...
    		d.queue.selected = j;
    		if ((d.queue.count + j) > f.fileNumLimit
    				&& f.fileNumLimit !== 0) {
    			...
    			if (typeof f.onError === "function") {
    				f.onError.call($this, "QUEUE_LIMIT_EXCEEDED");
    			}
    		} else {
    			for (var o = 0; o < j; o++) {
    				file = l.files[o];
    	...
    	if (f.itemTemplate == false) {
    		d.queueItem = $('<div class="uploadify-queue-item"><div class="cancel"><a class="close" href="#"></a></div><div>'
    	...
    	d.removeQueueItem = function(m, l, k) {
    		...
    		if (m.queueItem) {
    			...
    			if (typeof f.onDeleteSuccess === "function") {
    				f.onDeleteSuccess.call($this, fileName);
    			}
    		}
    	};
    	...
    	d.progress = function(l, j) {
    		if ($.inArray("onProgress", f.overrideEvents) < 0) {
    			...
    			j.queueItem.find(".uploadify-progress-bar").css("width",k + "%");
    		}
    		if (typeof f.onProgress === "function") {
    			f.onProgress.call($this, j, l);
    		}
    	};
    	d.error = function(l, j, k) {
    		...
    		if ($.inArray("onError", f.overrideEvents) < 0) {
    			...
    			alert(errorMsg);
               	j.queueItem.delay(1000 + 100 * delay).fadeOut(500,
    				function() {
    					$(this).remove();
    				});
    		}
    		if (typeof f.onError === "function") {
    			f.onError.call($this, l, j);
    		}
    		j.skip = true;
    	...
    	d.uploadComplete = function(l, j, k) {
    		if ($.inArray("onUploadSuccess", f.overrideEvents) < 0) {
    			j.queueItem.find(".uploadify-progress-bar").css("width",
    					"100%");
    		...
    		if (typeof f.onUploadSuccess === "function") {
    			f.onUploadSuccess.call($this, j,data);
    		}
    		var target = $('#' + f.id);
            ...
            for (var i = 0; i < fileData.length; i++) {
                ...
                if(fileLength>24){
                    file.FILENAME=file.FILENAME.substring(0,8)+"..."+file.FILENAME.substring(file.FILENAME.lastIndexOf('.')-3,file.FILENAME.length);
                }
                ...
                fileSpan.append('<a href="javascript:void(0)" FILENAME="' + file.FILENAME + '" FILECON="' + file.FILECON + '">' + file.FILENAME + '    (' + file["FILESIZE"] + ')</a>');
                fileWrapper.append(cancel).append(fileSpan).addClass("complete");;
                d.queueEl.append(fileWrapper);
                ...
                j.queueItem.fadeOut(500,function(){
                	$(this).remove();
                });
            }            	

            d.queueEl.find('span a').click(function() {

                window.open("/kjdp_download?DIR=" + f.dir + "&FILENAME=" + $(this).attr('FILENAME') + "&FILECON=" + $(this).attr('FILECON')
                		+"&IS_IMAGE_RESOURCE="+f.IS_IMAGE_RESOURCE +"&LOCAL_RESOURCE"+f.LOCAL_RESOURCE +"&UN_RENAME="+f.UN_RENAME);
            });
            
            
            var $this = this;
            d.queueEl.find('div.cancel:last a').click(function(e) {
                var FILECON = $(this).attr('FILECON'),
                    FILENAME = $(this).attr('FILENAME'),
                    ATT_ID = $(this).attr('ATT_ID');
                //$.get("/kjdp_delete?DIR=" + f.dir + "&FILENAME=" + FILENAME + "&FILECON=" + FILECON + "&ATT_ID=" + ATT_ID
                //    +'&IS_IMAGE_RESOURCE=' + f.IS_IMAGE_RESOURCE + '&LOCAL_RESOURCE=' + f.LOCAL_RESOURCE + '&UN_RENAME=' + f.UN_RENAME + '&SERVICE=P0004014', {},
                $.get("/kjdp_delete?DIR=" + f.dir + "&FILENAME=" + FILENAME + "&FILECON=" + FILECON + "&ATT_ID=" + ATT_ID
                    + '&IS_IMAGE_RESOURCE=' + f.IS_IMAGE_RESOURCE + '&LOCAL_RESOURCE=' + f.LOCAL_RESOURCE + '&UN_RENAME=' + f.UN_RENAME + '&SERVICE=U2410002&TABLENAME=ATTACH_FILE&COLNAME=ATT_CON', {},
                    function(data, textStatus) {
                        var hiddenIpt = $(target).find('input[name][name=' + targetName + ']');
                    var hiddenIptVal = hiddenIpt.val();
                    if (hiddenIptVal && hiddenIptVal != '') {
                ...
                if ((d.queue.count + l) > f.fileNumLimit && f.fileNumLimit !== 0) {
                    $(target).uploadify("disable", false);
                }
                ...
                if (typeof f.onDeleteSuccess === "function") f.onDeleteSuccess.call(this, file, data, k);
    	...
    	d.disable = function(isDisabled){
    		var btn = $(".uploadify-button");
    		var fileInput = $(".uploadify").find("input[type='file']");
    		if (isDisabled) {
    	...
    	if (window.File && window.FileList && window.Blob
    		...
    		var textStr = f.showButton ? f.buttonText:"";
    		
    		d.button = $('<div id="' + f.id +'">'
    				+ textStr + "</div>");
    		if (f.showButton){
    			d.button.addClass("uploadify-button");
    			if(f.buttonClass) {
    		...
    		//点击上传触发onDialogOpen
    		$(".uploadify").find("input[type='file']").bind("click",function(){
    			if (typeof f.onDialogOpen === "function") {
    				f.onDialogOpen.call($this);
    			}
    		});
    ...
    FlashUpload.prototype.init = function(target,options, swfUploadOptions){
    	if(options.autoCheck && !methods.isFlashAvailable()) {
    		$(document).hide();
            document.write('<div style="color: red;font-weight: bold;font-size: large;height: 30px;">Flash未启用或未安装</div>');
            document.write('<embed src="xx.swf" quality="high" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash" width="'+document.body.offsetWidth+'" height="'+(document.body.offsetHeight-30)+'">');
    		//alert(options.configFlash);
    	...
    	if($.browser.msie || $.browser.webkit)$(window).unload(function (){$("object[id*=SWFUpload_]",'body').parent().remove(".swfupload")});
    	var dir = options.dir || '';
    	...
    	return $this.each(function() {
    		...
    		var settings = $.extend({
    			...
    			swf: '../../kui-base/swf/uploadify.swf',
    		...
    		if(settings.showButton) {
    			...
    			$button.html('<span class="uploadify-button-text">' + settings.buttonText + '</span>').css({
                    ...
                    if($(this).hasClass("disabled")) {
                        $alert("您上传的的文件数量已经达到最大值，不能再继续上传！请先删除已上传的文件后再试。");
                    }
                });
    		} else {
    			$button = $();
    		...
    		if (settings.fileData && settings.fileData.length) {
    			...
    			return self.showFiles.call($this, settings.fileData);
    		}
    ...
    FlashUpload.prototype.getHandlers = function(){
    	return {
            ...
            onSelect: function(file) {
                ...
                for (var n in this.queueData.files) {
                    ...
                    if (queuedFile.uploaded != true && queuedFile.name == file.name) {
                        ...
                        var replaceQueueItem = $.message.confirm('提示','文件名为"' + file.name + '"的文件已经存在队列中.\n你想替换队列中存在着的文件吗?');
                        if (!replaceQueueItem) {
            ...
            onUploadSuccess: function(file, data, response) {
                ...
                this.queue.find('span:last a').click(function() {
                    window.open("/kjdp_download?DIR=" + settings.dir + "&FILENAME=" + $(this).attr('FILENAME') + "&FILECON=" + $(this).attr('FILECON') + "&IS_IMAGE_RESOURCE=" + settings.IS_IMAGE_RESOURCE + "&LOCAL_RESOURCE=" + settings.LOCAL_RESOURCE + "&UN_RENAME=" + settings.UN_RENAME);
                });
    			
                var $this = this;
                this.queue.find('div.cancel:last a').click(function(e) {
                    var FILECON = $(this).attr('FILECON'),
                    FILENAME = $(this).attr('FILENAME'),
                    ATT_ID = $(this).attr('ATT_ID');
                    ...
                    $.get("/kjdp_delete?DIR=" + settings.dir + "&FILENAME=" + FILENAME + "&FILECON=" + FILECON + "&ATT_ID=" + ATT_ID + 
                    		"&IS_IMAGE_RESOURCE="+settings.IS_IMAGE_RESOURCE +"&LOCAL_RESOURCE="+settings.LOCAL_RESOURCE +"&UN_RENAME="+settings.UN_RENAME+"&SERVICE=U2410002&TABLENAME=ATTACH_FILE&COLNAME=ATT_CON", {},
                    function(data, textStatus) {
                        var hiddenIpt = $(target).find('input[name][name=' + targetName + ']');
                        var hiddenIptVal = hiddenIpt.val();
                        if (hiddenIptVal != '') {
    				...
    				if(settings.fileNumLimit){
    					if($this.queueData.queueLength<settings.fileNumLimit){
    						$(target).uploadify("disable",false);
    					}
    				}
                    $(this).parent().parent().remove();
    			...
    			swfuploadify.queue.find('div.cancel a').click(function(e) {
    				...
    				var deleteFile = function(){
                        ...
                        function(data, textStatus) {
                            ...
                            if (settings.onDeleteSuccess) settings.onDeleteSuccess.call(that, file, data, response);
                        });
                    }
                    confirm('提示', '删除的文件无法恢复，您确认删除吗?', function (flag) {
                        if (flag) {
                            try {
                                deleteFile();
                            } catch (e) {
