...
(function($) {
    ...
    $.fn.resizable = function(options, param) {
            ...
            if (resizeData.dir.indexOf('w') != -1) {
                ...
                if (width >= options.minWidth && width <= options.maxWidth) {
                    resizeData.left = resizeData.startLeft + e.pageX - resizeData.startX;
                	 ...
                	 resizeData.left = resizeData.startLeft + resizeData.startWidth - Math.min(options.minWidth, options.maxWidth);
                }
                resizeData.width = Math.min(Math.max(width, options.minWidth), options.maxWidth);
            }
            if (resizeData.dir.indexOf('n') != -1) {
                ...
                if (height >= options.minHeight && height <= options.maxHeight) {
                    resizeData.top = resizeData.startTop + e.pageY - resizeData.startY;
               	 	...
               	 	resizeData.top = resizeData.startTop + resizeData.startHeight - Math.min(options.minHeight, options.maxHeight);
                }
                resizeData.height = Math.min(Math.max(height, options.minHeight), options.maxHeight);
            }
        }
        ...
        return this.each(function() {
            ...
            if (state) {
                ...
                opts = $.extend({},
                $.fn.resizable.defaults, $.fn.resizable.parseOptions(this), options || {});
                $.data(this, 'resizable', {
                    options: opts
                });
            }
    ...
    $.fn.resizable.parseOptions = function(target) {
        var t = $(target);
        return $.extend({},
        $.parser.parseOptions(target, ['handles', {
            minWidth: 'number',
            minHeight: 'number',
            maxWidth: 'number',
            maxHeight: 'number',
