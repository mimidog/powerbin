...
function init(){
  ...
  // Else if this is a newly created model
    ...
    new ORYX.Editor({ id:'oryx-canvas123',fullscreen:true,stencilset:{ url:"/oryx" + ORYX.Utils.getParamFromUrl("stencilset") } });
  }
...
ORYX.Editor = {
  ...
  _generateGUI:function(){
    ...
    Ext.layout.BorderLayout.Region.prototype.getCollapsedEl = function(){
      ...
      if(this.collapseTitle){
        ...
        var svg = ORYX.Editor.graft("http://www.w3.org/2000/svg",this.collapsedEl.dom,['svg',{style:"position:relative;left:" + (this.position === "west" ? 4 : 6) + "px;top:" + (this.position === "west" ? 2 : 5) + "px;"},['text',{transform:"rotate(90)",x:0,y:0,"stroke-width":"0px",fill:"#EEEEEE",style:"font-weight:bold;","font-size":"11"},this.collapseTitle]
        ]);
    ...
    var layout_config = {
      layout:'border',
      items:[
        this.layout_regions.north,this.layout_regions.east,this.layout_regions.south,this.layout_regions.west,this.layout_regions.center
      ]
    }
  ...
  _generateHeader:function(){
    ...
    var fn = function(val){
      ...
      var content = "<div id='oryx_editor_header'>" + "<a href=\"" + ORYX.CONFIG.WEB_URL + "\" target=\"_blank\">" + "<img src='" + ORYX.PATH + "images/oryx.small.gif' border=\"0\" />" + "</a>" + "<span class='openid " + (publicText == user ? "not" : "") + "'>" + (unescape(user)) + maModelAuthI + "</span>" + "<div style='clear: both;'/>" + "</div>";
      if(headerPanel.body){
