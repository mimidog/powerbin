...
(function($){
	...
	$.fn.searchbox.methods = {
		...
		clear:function(){
			$(this).searchbox('options').value = "";
		  $(this).searchbox('textbox').val("");
			$(this).searchbox('textbox').blur();
			},
		getName: function(jq){
	...
	$.fn.searchbox.parseOptions = function(target){
		var t = $(target);	
		return  $.extend({},$.parser.parseOptions(target, ['width', 'prompt', 'value', 'menu', 'searcher']),{width: (parseInt(target.style.width) || undefined),value: t.val()});
	};
